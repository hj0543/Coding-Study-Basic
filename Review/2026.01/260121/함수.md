...
* 정의(Definition): def 키워드로 시작

* 본문(Body): 들여쓰기(Indentation) 필수

* 설명(Docstring): 함수 본문 첫 줄의 선택적 문서화 문자열 ("""...""")
...
## 3. 매개변수와 인자 (두 번째 고비)
* 매개변수 : 함수를 정의할 때 사용하는 변수명
  -> ★ 어떤 데이터인지 알 수 있는 명확한 이름
* 인자 : 함수를 호출할 때 실제 전달하는 값
...
### 1) 인자의 종류
**위치 인자 (Positional)**: 순서대로 전달 (필수)

**기본 인자 (Default)**: 값이 없으면 설정된 기본값 사용

**키워드 인자 (Keyword)**: 이름을 명시하여 전달 (순서 무관)
